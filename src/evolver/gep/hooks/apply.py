...
def _replace_function_body(source: str, func_name: str, new_body: str) -> str:
    ...
    if func_name not in spans:
        raise MechanismContractError(f"surface has no top-level function {func_name!r}")
...
def _apply_prompt_instruction(source: str, hook: str, value: Any) -> str:
    """Rewrite a prompt-builder function body (hook → function name)."""
    if not isinstance(value, str) or not value.strip():
        raise MechanismContractError(f"prompt_instruction hook {hook!r} requires non-empty source")
...
def _apply_gene_library(gene: dict[str, Any], hook: str, value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, list) or not all(isinstance(v, str) for v in value):
        raise MechanismContractError("gene_signal_alias requires a list of signal strings")
    ...
    *surface* is the family's editable object: a Python source ``str`` for
    ...
    if mechanism_family == "prompt_instruction":
        if not isinstance(surface, str):
            raise MechanismContractError("prompt_instruction surface must be Python source (str)")
        return _apply_prompt_instruction(surface, hook, value)
    if mechanism_family == "gene_library":
        if not isinstance(surface, dict):
            raise MechanismContractError("gene_library surface must be a gene record (dict)")
        return _apply_gene_library(surface, hook, value)
    raise MechanismContractError(f"mechanism_family {mechanism_family!r} not yet implemented")
