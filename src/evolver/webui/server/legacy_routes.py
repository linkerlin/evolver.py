"""Legacy WebUI routes kept for backward compatibility.

Pre-observer API surface: ``/status``, ``/events``, ``/genes``, ``/ws``, etc.
New dashboards should prefer ``/api/*`` from :mod:`evolver.webui.server.routes`.
"""
...
async def events_replay(since_id: int = 0, limit: int = 100) -> JSONResponse:
    ...
    replayed = read_events_replay(since_id, limit)
    return JSONResponse({"events": replayed, "since_id": since_id})
