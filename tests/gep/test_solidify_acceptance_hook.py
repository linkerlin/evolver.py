...
def git_ws(temp_workspace: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    # git_ops.rollback_tracked()/stash run git in Path.cwd() when no cwd arg is
    # given (solidify calls it without one) — mirror production by running the
    # whole test with the temp workspace as process cwd.
