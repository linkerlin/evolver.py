...
class TestParseSkillMdGovernance:
    ...
    def test_keeps_human_gate_items(self) -> None:
        blob = " ".join(self.parsed.strategy).lower()
        assert (
            "human confirmation" in blob
            or "long-term memory" in blob
            or "production strategy" in blob
        )
    ...
    def test_uniform_indented_list_stays_separate(self) -> None:
        md = "\n".join(
            [
                "---",
                "name: t",
                "description: optimize the build",
                "---",
                "# T",
                "",
                "## Workflow",
...
class TestInferCategory:
    def test_upgrade_with_rollback_is_optimize(self) -> None:
        assert (
            infer_category(
                [], "Use when upgrading an AI system with versioning and rollback and guard rails"
            )
            == "optimize"
        )
